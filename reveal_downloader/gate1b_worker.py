...
from .hd_request_worker import run_worker as run_hd_request_worker
...
def handle_cron(
    ...
    *,
    ...
    if hd_configured:
        # Gate 1B's database trigger creates durable HD queue rows. Drain them in
        # the same scheduled invocation so an external workflow is not required
        # to bridge classification to Reveal submission.
        hd_requests = run_hd_request_worker(
            environ, max_requests=20, deadline_seconds=50.0
        )
        result["hd_requests"] = hd_requests
        if not hd_requests.get("ok"):
            return 503, {
                "ok": False,
                "error": "HD request batch incomplete",
                "hd_requests": hd_requests,
            }
    return 200, result
