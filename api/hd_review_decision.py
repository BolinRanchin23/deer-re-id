...
class handler(BaseHTTPRequestHandler):
 def do_POST(self):
  ...
  status,result=handle_hd_review_decision(os.environ,payload.get("action_token",""),payload.get("action",""),instance_id=payload.get("hd_animal_instance_id",""),profile_id=payload.get("profile_id",""),display_name=payload.get("display_name",""),species=payload.get("species",""),sex=payload.get("sex",""),note=payload.get("note","")); self._write(status,result)
