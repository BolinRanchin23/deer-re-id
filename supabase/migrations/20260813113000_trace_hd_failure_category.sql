...
select jsonb_build_object(
  'media_id',m.id,'captured_at',m.captured_at,'variant',m.variant,
  'assets',coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'failure',coalesce((select jsonb_build_object('error_category',f.error_category,'created_at',f.created_at) from deerid.hd_review_failures f where f.media_asset_id=a.id limit 1),'null'::jsonb),
