-- Follow-up fixes from independent security/correctness review.

alter table deerid.collections
  drop constraint if exists collections_collection_type_provider_collection_id_key;
create unique index if not exists collections_provider_identity_idx
  on deerid.collections (collection_type, provider_collection_id)
  where provider_collection_id is not null;

alter table deerid.media_weather
  add column if not exists feels_like numeric,
  add column if not exists humidity numeric check (humidity is null or humidity between 0 and 100);
  ...
  for item in select value from jsonb_array_elements(coalesce(p_media, '[]'::jsonb)) loop
    ...
    if jsonb_typeof(weather) = 'object' then
      ...
      on conflict (media_id) do update set
        ...
        temperature = excluded.temperature, feels_like = excluded.feels_like, humidity = excluded.humidity,
