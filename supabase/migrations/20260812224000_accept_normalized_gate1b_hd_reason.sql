-- Accept the normalized Gate 1B priority reason written by the legacy priority trigger.
...
declare request_row deerid.hd_requests%rowtype; decision_id bigint; advanced_id bigint;
begin
 ...
 if request_row.review_version is null and request_row.priority_reason in
   ('gate1b_likely_male','gate1b_automatic_likely_male','gate1b_audit_correction') then
   ...
   values(request_row.media_id,request_row.gate1_assessment_id,request_row.review_version,'request_hd',request_row.pending_note)
...
-- Preserve the second ambiguous provider outcome; never auto-resend it.
update deerid.hd_requests set status='unknown',attempts=attempts+1,last_error='provider_outcome_unknown',
 request_token=null,request_started_at=null,updated_at=now()
where status='requesting' and review_version is null
 and priority_reason in ('gate1b_likely_male','gate1b_automatic_likely_male','gate1b_audit_correction');
